...
- 比對這些變動檔案與 commit message，自動與今日的 To-do 項目做關聯，分析出各項目具體修改了哪些程式檔案與功能。

### 4. 生成並輸出三層結構的進度報告
將結果整理為以下三層結構。**注意：此輸出最後不要附上任何使用了哪些 Agent 技能或規定的備註。**

*   **第一層：目前進度概況** (給外人看的大綱，標示各項目的自動對齊進度。**必須具體寫出今日所完成的實質工作摘要與修改的核心檔案名稱，拒絕僅寫標籤狀態**)
*   **第二層：今日 Git 變更與提交明細** (列出今日各專案實際的 commit 清單與目前工作區修改的**具體檔案與路徑**，提供核對來源)
...
- 項目1 (狀態：已有 Commit 提交)
  * 實作說明與修改的代碼檔案名稱 (例如: 實作遠端控制通道, 修改 [FeatureWheel.swift](file://...))
- 項目2 (狀態：工作區修改中)
  * 實作說明與修改的代碼檔案名稱
- 項目3 (狀態：尚無變動)
- 新項目A (狀態：工作區修改中)
